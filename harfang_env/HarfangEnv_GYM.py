...
class HarfangEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(low=np.array([-1.0] * 14), high=np.array([1.0] * 14), dtype=np.float64)
    ...
    def step(self, action_ally):
        ...
        reward = self._get_reward()  # get reward value
        self._get_termination()  # check termination conditions
        info = {"success": self.episode_success, "fire_success": self.fire_success}

        return np.array(state_ally, dtype=np.float64), reward, self.done, info
    ...
    def step_test(self, action_ally):
        ...
        return np.array(state_ally, dtype=np.float64), reward, self.done, info, self.now_missile_state, self.missile1_state, self.n_missile1_state, self.Ally_target_locked, self.success
...
class InfiniteHarfangEnv(HarfangEnv):
    ...
    def step(self, action_ally):
        self.episode_step += 1
        if self.episode_step % 50 == 0: # 每50 step 装一次弹
        ...
        info = {"success": self.episode_success, "fire_success": self.fire_success}
        return state_ally, self.reward, self.done, info

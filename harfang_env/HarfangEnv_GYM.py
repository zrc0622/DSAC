...
from harfang_env.Constants import *
import gym
import os
import inspect
import random
...
class HarfangEnv(gym.Env):
    def __init__(self):
        super(HarfangEnv, self).__init__()
        self.done = False
        self.loc_diff = 0
        
        # gym
        self.action_space = gym.spaces.Box(low=np.array([-1.0, -1.0, -1.0, -1.0]), high=np.array([1.0, 1.0, 1.0, 1.0]), dtype=np.float64)
        self.observation_space = gym.spaces.Box(low=np.array([-1.0] * 13), high=np.array([1.0] * 13), dtype=np.float64)
    ...
    def random_reset(self):  # reset simulation beginning of episode
        ...
        return np.array(state_ally, dtype=np.float64)
    ...
    def step(self, action):
        ...
        n_state = self._get_observation()  # in each step, get observation
        reward = self._get_reward(self.state, action, n_state)  # get reward value
        self.state = n_state
        self._get_termination()  # check termination conditions
        info = {"success": self.episode_success, "fire_success": self.fire_success, "TimeLimit.truncated": timelimit}

        return np.array(n_state, dtype=np.float64), reward, self.done, info
    ...
    def step_test(self, action):
        ...
        return np.array(n_state, dtype=np.float64), reward, self.done, info, self.now_missile_state, self.missile1_state, self.n_missile1_state, self.Ally_target_locked, self.success
    ...
    def _get_reward(self, state, action, n_state):
        ...
        if self.now_missile_state == True: # 如果此step导弹发射
            ...
            if self.missile1_state == True and self.Ally_target_locked == False: # 且导弹存在、不锁敌
                # self.reward -= 4 # 4、4
                self.success = -1
                print(f'failed to fire: {self.episode_steps}')
